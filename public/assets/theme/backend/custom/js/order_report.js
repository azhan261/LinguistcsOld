...
$(document).on("click", "#saveProduct", function () {
    ...
    product = product + '<span id="additional_amnt' + totalProducts + '"> ' + additionalAmount + '</span>';
    ...
    product = product + '<td class="text-right">';
    product = product + '<small>RS</small>';
    product = product + '<span id="total' + totalProducts + '"> ' + totalAmount + '</span>';
...
function ShowOrderPreview() {
    ...
    $('#tab-cart table >  tbody  > tr').each(function (key, value) {
        ...
        if($(this).find("#unitPriceVal" + index).val()) {
            ...
            string = string + '<td class="text-right"><small>Rs</small> ' + unitPrice + '</td>';
            string = string + '<td class="text-right"><small>Rs</small> ' + additionalAmount + '</td>';
            string = string + '<td class="text-right"><small>Rs</small> ' + total + '</td>';
            string = string + '</tr>';
        }
    });
    if (!string) {
    ...
    Total = subTotal;
    string = string + '<tr>';
    string = string + '<td class="text-right" colspan="5">Sub-Total:</td>';
    string = string + '<td class="text-right" id="PreviewSubTotal"><small>Rs</small> ' + subTotal + '</td>';
    string = string + '</tr>';
    string = string + '<tr>';
    string = string + '<td class="text-right" colspan="5">Total:</td>';
    string = string + '<td class="text-right" id="PreviewTotal"><small>Rs</small> ' + Total + '</td>';
    string = string + '</tr>';
    $("#total").html(string);
}
...
function ExportOrders(){
    ...
    if($("#search_customer_field_filter").val())
        $("#rp_search_customer_field_filter").val($("#search_customer_field_filter").val());
    else
       $("#rp_search_customer_field_filter").val('');


    if($("#search_order_id_field_filter").val())
        $("#rp_search_order_id_field_filter").val($("#search_order_id_field_filter").val());
    else
       $("#rp_search_order_id_field_filter").val();
    

    if($("#filter_status").val())
        $("#rp_filter_status").val($("#filter_status").val());
    else
       $("#rp_filter_status").val();
    ...
    if($("#sort_order").val())
        $("#rp_sort_order").val($("#sort_order").val());
    else
        $("#rp_sort_order").val();
    ...
    if($("#to_date").val())
       $('#rp_to_date').val($("#to_date").val());
    else
       $('#rp_to_date').val('');
   
    //alert($("#export_orders").attr('action'));
