...
$(function () {
    ...
    $("#addCatForm").validate({
        ...
        submitHandler: function (form) {
            ...
            form_data.append('amount', $('#p_amount').val());
            ...
            $.ajax({
                url: APP_URL + "/admin/save-pay-plan",
                data: form_data,
                type: "post",
                cache: false,
                contentType: false,
         ...
         $.ajax({
                ...
                success:function(response){
                    ...
                    if(response.plans){
                      ...
                      $("#cid").val(response.plans.id);
                      $("#title").val(response.plans.title);
                      $("#p_amount").val(response.plans.amount);
                      $("#course_id").val(response.plans.course_id).change();
                      $("#level_no").val(response.plans.level_no).change();
                      $("#status").val(response.plans.status).change();
                      $("#addCatModal").modal('show');
                  }
...
function TrashCat(event){
    ...
    if(id){
        ...
        if(res){
            $.ajax({
                url:APP_URL + "/admin/delete_pay_plan",
                data:{'id':id,'_token':csrfTokenVal},
                dataType:'json',
                type:'post',
                beforeSend: function () {
