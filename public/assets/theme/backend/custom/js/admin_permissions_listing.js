...
function ResetPermissions(){
    ...
    var url = $(location).attr('href');
    var page_number = get_parameter_val("page", url);
    $("#admin_permission_search_field_filter").val('');
    var url = window.location.href;
    var url = updateQueryStringParameter(url, "page", 1);
    var data = make_final_parameters_object(url);
    data = makeDataObject(data);
...
function getPermissions(url, data) {
    block_gui_start();
    $.ajax({
        data: data,
        url: url,
        success: function (data){
            block_gui_end();
            $('#admin_permissions_listing_block').html(data);
...
function makeDataObject(data){
    ...
    if($("#admin_permission_search_field_filter").val())
       data['search'] = $("#admin_permission_search_field_filter").val();
    else
       data['search'] = "";
         ...
         $("#add_permission_form").validate({
            ...
            submitHandler: function(form){
                ...
                $("#save_permission").prop("disabled",true);
                    var data =  $("#add_permission_form").serialize()+ '&roles=' + $('#roles').val();
                    $.ajax({
                        url:APP_URL+"/admin/permissions/save",
                        data:data,
                        dataType:"json",
                        type:"post",
                        beforeSend: function(){
