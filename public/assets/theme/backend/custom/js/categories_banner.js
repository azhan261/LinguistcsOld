...
$(function () {
    ...
    $("#addCatForm").validate({
        ...
        submitHandler: function (form) {
            ...
            form_data.append('banner_url', $('#url').val());
            ...
            $.ajax({
                url: APP_URL + "/admin/save_category_banner",
                data: form_data,
                type: "post",
                cache: false,
                contentType: false,
...
function UpdateCat(event){
     ...
     if(id){
         $.ajax({
                url:APP_URL + "/admin/get_category_banner",
                ...
                success:function(response){
                    ...
                    if(response.category){
                      ...
                      $("#url").val(response.category.banner_url);
                      $("#parent_id").val(response.category.category_id).change();
                      ...
                      $("#desktop_images").html('');
                                ...
                                if(response.category.banner_image_url){
                                    var mediaItem = '';  
                                    var mediaItem = '<div class="thumbnail col-md-2" id="p_'+response.category.id+'">';
                                    var imgUrl = APP_URL + '/public/upload/'+response.category.banner_image_url;
                                 ...
                                 if(response.category.banner_mb_image_url){
                                    var mediaItem = '';  
                                    var mediaItem = '<div class="thumbnail col-md-2" id="p_'+response.category.id+'">';
                                    var imgUrl = APP_URL + '/public/upload/'+response.category.banner_mb_image_url;
                                    //mediaItem = mediaItem + '<a class="close" href="javascript:void(0)" id="img_'+value.id+'"  onclick="RemoveMedia('+value.id+')">×</a>';
                                    mediaItem = mediaItem +'<img src="'+imgUrl+'">';
                                    mediaItem = mediaItem +'</div>';
...
function TrashCat(event){
    ...
    if(id){
        ...
        if(res){
            $.ajax({
                url:APP_URL + "/admin/trash_category_banner",
                data:{'id':id,'_token':csrfTokenVal},
                dataType:'json',
                type:'post',
                beforeSend: function () {
